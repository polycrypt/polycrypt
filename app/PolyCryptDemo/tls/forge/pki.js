...
pki.verifyCertificateChain = function(caStore, chain, verify) {
  ...
  do {
    ...
    // 2. verify with parent
      ...
      var verified = false;
      if(chain.length > 0) {
        console.log("Verifying using parent");
        // verify using parent
        parent = chain[0];
        try {
      ...
      // get parent(s) from CA store
        ...
        console.log("Getting parent from CA store");
        ...
        if(parents === null) {
          ...
          while(!verified && parents.length > 0) {
            ...
            try {
              verified = parent.verify(cert);
              console.log("Got verification result: "+verified);
            }
            catch(ex) {
                console.log("Verification failure");
                console.log(ex.stack);
...
pki.encryptPrivateKeyInfo = function(obj, password, options) {
  ...
  var encAlgo;
  var encVal;
  if (options.encAlg.indexOf('aes') === 0) {
    // Do PBES2
    var encOid; 
    if(options.encAlg === 'aes128') {
      ...
      throw { message: "Unknown encryption algorithm " + options.encAlg };
    }
    ...
    console.log("Key constructor: " + dk.constructor);
    var iv = forge.random.getBytes(16);
    var cipher = forge.aes.createEncryptionCipher(dk);
    cipher.start(iv);
    cipher.update(asn1.toDer(obj));
    cipher.finish();
    encVal = cipher.output.getBytes();

    encAlgo = asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
      asn1.create(asn1.Class.UNIVERSAL, asn1.Type.OID, false,
  ...
  var rval = asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
    ...
    encAlgo,
    // encryptedData
    asn1.create(asn1.Class.UNIVERSAL, asn1.Type.OCTETSTRING, false, encVal)
  ]);
  return rval;
};
